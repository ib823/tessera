...
Audit of all 70 published issues against the new accuracy + editorial criteria from CLAUDE.md (Accuracy Standard) and `docs/research/bite-size-reading.md`. Generated by `scripts/audit-published.mjs`.

## Summary

| Metric | Value |
|---|---:|
| Total published issues | 70 |
...
| **Unauditable, undisclosed (no badge, no audit trail)** | **32** |
...
### Unauditable issues — zero engine artifacts (32)
...
- **32 undisclosed** — no badge yet. These should either get the flag or full pipeline ASAP.
...
- **1995** — Singapore charged a fourth in the Nvidia chip case. No Malaysian operator faces charges yet.
...
*No hits across all 70 published issues.* (Note: this means the published corpus is already clean of these specific phrases — surprising and worth verifying the patterns are correctly tuned.)
...
2. **For unauditable issues without other flags** (count: 32) — editorial decision per issue: backfill audit trail (re-run pipeline), keep as legacy with disclaimer, or unpublish. Do NOT auto-act.
