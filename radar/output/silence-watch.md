# Radar — Silence Watch

Scan: 2026-06-09 18:52 UTC. Queue size: 787. Picks shown: 25.

These are structurally important events that have accumulated in the
radar queue with unexpectedly low coverage. Rank score combines
silence × structural importance × age — items that have been silent
for weeks rise above fresh-but-quiet items. Already-published topics
and in-flight briefs are filtered out.

Auto-generated by `radar/scripts/build-silence-watch.py`. The
curated `top-issues-to-develop.md` should pick at least one item
from this list per scan cycle — otherwise underreported issues
compound into a timebomb the radar warned about but T4A never
developed.


---


### S1. kkm akan kemuka cadangan jimat kos tanpa jejas perkhidmatan penting. ia susulan keputusan kerajaan memperti...

- **Rank score:** 1.015 (silence=1.00 × importance=0.51 × age_factor)

- **Age in queue:** 35.9 days (source date: 2026-05-04, first seen: 2026-05-04)

- **Coverage:** 1 news, 2 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S2. suhaili gagal cabar tindakan bersatu tamat keahlian. panel mahkamah rayuan sebulat suara tolak rayuan ahli ...

- **Rank score:** 1.000 (silence=0.99 × importance=0.51 × age_factor)

- **Age in queue:** 40.9 days (source date: 2026-04-29, first seen: 2026-04-29)

- **Coverage:** 2 news, 4 social — pattern: _below-expected coverage_

- **Dimensions at risk:** legal, political


### S3. hak 40% hasil persekutuan: keputusan permohonan tangguh perintah isnin depan. panel 3 hakim mahkamah rayuan...

- **Rank score:** 0.997 (silence=0.99 × importance=0.51 × age_factor)

- **Age in queue:** 69.9 days (source date: 2026-03-31, first seen: 2026-03-31)

- **Coverage:** 2 news, 6 social — pattern: _below-expected coverage_

- **Dimensions at risk:** legal, political


### S4. budget 2027 presentation

- **Rank score:** 0.977 (silence=1.00 × importance=0.54 × age_factor)

- **Age in queue:** 24.3 days (source date: 2026-06-09, first seen: 2026-05-16)

- **Coverage:** 1 news, 0 social — pattern: _news only — no social_

- **Dimensions at risk:** economic, political


### S5. parliament budget session session opens

- **Rank score:** 0.927 (silence=0.99 × importance=0.52 × age_factor)

- **Age in queue:** 24.3 days (source date: 2026-06-09, first seen: 2026-05-16)

- **Coverage:** 3 news, 1 social — pattern: _below-expected coverage_

- **Dimensions at risk:** economic, political


### S6. sprm akan rayu keputusan mahkamah tolak perintah larangan akses rm548 juta. sprm kata notis rayuan akan dif...

- **Rank score:** 0.853 (silence=0.99 × importance=0.54 × age_factor)

- **Age in queue:** 18.2 days (source date: 2026-05-22, first seen: 2026-05-22)

- **Coverage:** 6 news, 4 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, legal


### S7. parliament second session session opens

- **Rank score:** 0.833 (silence=0.99 × importance=0.47 × age_factor)

- **Age in queue:** 24.3 days (source date: 2026-06-09, first seen: 2026-05-16)

- **Coverage:** 4 news, 1 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S8. selesai disoal 9 jam, rafizi beredar dari sprm. ahli parlimen pandan itu tiba di ibu pejabat sprm pada 9.30...

- **Rank score:** 0.803 (silence=0.98 × importance=0.41 × age_factor)

- **Age in queue:** 35.9 days (source date: 2026-05-04, first seen: 2026-05-04)

- **Coverage:** 2 news, 3 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, political


### S9. selepas dap, pemuda mca umum sertai himpunan anti-azam baki. jawatankuasa pusat kata sudah tiba masa keraja...

- **Rank score:** 0.796 (silence=0.97 × importance=0.41 × age_factor)

- **Age in queue:** 46.9 days (source date: 2026-04-23, first seen: 2026-04-23)

- **Coverage:** 3 news, 4 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, political


### S10. sukar ‘terlupa’ bentang belanjawan 10 tahun berturut-turut. audit afc mendapati fam gagal membentangkan bel...

- **Rank score:** 0.688 (silence=1.00 × importance=0.61 × age_factor)

- **Age in queue:** 3.6 days (source date: 2026-06-06, first seen: 2026-06-06)

- **Coverage:** 2 news, 1 social — pattern: _below-expected coverage_

- **Dimensions at risk:** economic, institutional, political


### S11. syahredzan bidas bekas bos sprm, syor pertuduhan sebelum selesai siasat. ahli parlimen bangi berkata kenyat...

- **Rank score:** 0.653 (silence=0.97 × importance=0.41 × age_factor)

- **Age in queue:** 19.2 days (source date: 2026-05-21, first seen: 2026-05-21)

- **Coverage:** 4 news, 3 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, political


### S12. sprm siasat 2 individu berkait pengambilalihan ijm. ketua pesuruhjaya sprm abd halim aman berkata siasatan ...

- **Rank score:** 0.653 (silence=0.95 × importance=0.41 × age_factor)

- **Age in queue:** 20.1 days (source date: 2026-05-21, first seen: 2026-05-20)

- **Coverage:** 5 news, 6 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, political


### S13. ag mahu batal keputusan benarkan majlis peguam cabar dnaa zahid. peguam negara dusuki mokhtar kemuka dua pe...

- **Rank score:** 0.633 (silence=0.98 × importance=0.56 × age_factor)

- **Age in queue:** 4.3 days (source date: 2026-06-05, first seen: 2026-06-05)

- **Coverage:** 5 news, 11 social — pattern: _below-expected coverage_

- **Dimensions at risk:** legal


### S14. pemuda pas bangi lulus usul putus hubungan dengan bersatu. sayap itu juga gesa kepimpinan pas dan pemuda pa...

- **Rank score:** 0.625 (silence=0.99 × importance=0.56 × age_factor)

- **Age in queue:** 3.4 days (source date: 2026-06-06, first seen: 2026-06-06)

- **Coverage:** 3 news, 3 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S15. zara qairina inquest hears emotional mother-daughter audio recordings. kota kinabalu, may 18 &mdash; the co...

- **Rank score:** 0.616 (silence=0.86 × importance=0.41 × age_factor)

- **Age in queue:** 22.2 days (source date: 2026-05-19, first seen: 2026-05-18)

- **Coverage:** 18 news, 15 social — pattern: _below-expected coverage_

- **Dimensions at risk:** legal, political


### S16. warga emas antara ditahan bersama dadah rm57.15 juta. polis syak bekalan dadah mahu diseludup ke negara jir...

- **Rank score:** 0.608 (silence=0.99 × importance=0.55 × age_factor)

- **Age in queue:** 3.4 days (source date: 2026-06-06, first seen: 2026-06-06)

- **Coverage:** 2 news, 2 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S17. pm arah isu kelayakan pharmd lulusan universiti alexandria diselesai segera. kementerian pendidikan tinggi ...

- **Rank score:** 0.587 (silence=0.99 × importance=0.52 × age_factor)

- **Age in queue:** 4.4 days (source date: 2026-06-05, first seen: 2026-06-05)

- **Coverage:** 3 news, 2 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S18. 2 maut dalam protes kemudahan ebola as di kenya. protes itu susulan penubuhan pusat kuarantin ebola milik a...

- **Rank score:** 0.569 (silence=0.99 × importance=0.47 × age_factor)

- **Age in queue:** 7.1 days (source date: 2026-06-02, first seen: 2026-06-02)

- **Coverage:** 3 news, 2 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S19. wanita, 3 penjamin diperintah bayar pinjaman mara rm857,000. mahkamah tinggi putuskan penemuan mahkamah ses...

- **Rank score:** 0.568 (silence=0.99 × importance=0.48 × age_factor)

- **Age in queue:** 5.8 days (source date: 2026-06-03, first seen: 2026-06-03)

- **Coverage:** 2 news, 2 social — pattern: _below-expected coverage_

- **Dimensions at risk:** legal


### S20. bentang laporan penuh audit afc, ahli parlimen seru fam. ramkarpal singh berkata orang awam ada hak tahu se...

- **Rank score:** 0.560 (silence=0.98 × importance=0.52 × age_factor)

- **Age in queue:** 3.3 days (source date: 2026-06-06, first seen: 2026-06-06)

- **Coverage:** 8 news, 5 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional, political


### S21. 36 tahun jadi ahli parlimen serian, richard riot sasar penggal ke-9. ahli politik berusia 74 tahun itu berk...

- **Rank score:** 0.558 (silence=0.97 × importance=0.47 × age_factor)

- **Age in queue:** 7.1 days (source date: 2026-06-02, first seen: 2026-06-02)

- **Coverage:** 6 news, 8 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S22. saiful hazmi dilantik pengarah bahagian perundangan dan pendakwaan sprm. beliau menggantikan wan shaharuddi...

- **Rank score:** 0.553 (silence=0.99 × importance=0.47 × age_factor)

- **Age in queue:** 6.1 days (source date: 2026-06-03, first seen: 2026-06-03)

- **Coverage:** 2 news, 3 social — pattern: _below-expected coverage_

- **Dimensions at risk:** institutional


### S23. ahli parlimen sokong muafakat umno-pas, cadang masukkan gerakan hamzah. wan saiful wan jan berkata konsep f...

- **Rank score:** 0.537 (silence=0.98 × importance=0.47 × age_factor)

- **Age in queue:** 5.4 days (source date: 2026-06-04, first seen: 2026-06-04)

- **Coverage:** 3 news, 6 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S24. ahli pkr pertahan gantung akses mykhas bagi 2 ahli parlimen. timbalan setiausaha agung pkr, bryan ng, kata ...

- **Rank score:** 0.534 (silence=0.97 × importance=0.47 × age_factor)

- **Age in queue:** 5.4 days (source date: 2026-06-04, first seen: 2026-06-04)

- **Coverage:** 8 news, 4 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


### S25. pemimpin pkr pertahan gantung akses mykhas 2 ahli parlimen. timbalan setiausaha agung pkr, bryan ng, kata i...

- **Rank score:** 0.532 (silence=0.97 × importance=0.47 × age_factor)

- **Age in queue:** 5.3 days (source date: 2026-06-04, first seen: 2026-06-04)

- **Coverage:** 6 news, 6 social — pattern: _below-expected coverage_

- **Dimensions at risk:** political


---


## How to use this list


1. Open the top 3 and check whether each has a verifiable primary
   source (official document, court judgment, gazette, regulator
   publication). If not, drop it.
2. If it does, run the publish playbook against it — these are
   exactly the issues T4A exists to surface.
3. If the curator skips a silence pick for editorial reasons,
   note it in `top-issues-to-develop.md` under a Skip section so
   the next scan doesn't re-surface it indefinitely.
