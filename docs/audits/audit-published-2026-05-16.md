...
Audit of all 99 published issues against the new accuracy + editorial criteria from CLAUDE.md (Accuracy Standard) and `docs/research/bite-size-reading.md`. Generated by `scripts/audit-published.mjs`.

## Summary

| Metric | Value |
|---|---:|
| Total published issues | 99 |
| With full pipeline (≥7 of 8 artifacts) | 24 |
| Partially audited (1-6 artifacts) | 3 |
| **Unauditable (zero artifacts)** | **72** |
| Phase 1 structural flags | 5 issues |
| Phase 3 low-confidence (FAS<70 or m_true<0.7) | 25 issues |
| Phase 4 anti-pattern hits | 1 hits across 1 issues |
| Tier 1 review (highest priority) | 3 issues |
| Tier 2 review (low confidence) | 25 issues |
...
- **1984** `cards[1]` — 302 chars (max 300)
- **1984** `cards[5].big` — 183 chars (max 180)
- **1985** `cards[1]` — 313 chars (max 300)
...
### Stage 3 results — all issues with artifacts (26)
...
### Tier 2 — Low Stage 3 confidence (25)
